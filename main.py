...
for instrument in instrument_list:
    file_name = f'{instrument}_{from_time}_{to_time}.csv'
    
    for idx in range(chunks):
        start_count = count * idx
        ...
        try:
            print(f'[{round(100*start_count/number_of_candles)}%] Fetching {instrument} between {start_time_str} (inc.) and {end_time_str} (exc.)')
            s = requests.Session()
